...
const UpcomingListItem = ({ title, date, image, id }: { title: string, date: string, image: string, id: number }) => (
  <Link href={`/anime/${id}`} className="flex items-center gap-3 p-2 rounded-lg hover:bg-secondary/50 transition-colors group">
    ...
    <div className="min-w-0 flex-1">
      <h4 className="text-sm font-medium line-clamp-2 group-hover:text-primary transition-colors">{title}</h4>
        ...
        {isLoading ? (
          ...
          <div className="mb-10 text-left">
            ...
            <p className="text-muted-foreground">
              ...
              Keep track of your anime collection here.
            </p>
          </div>
        )}
              ...
              <section className="bg-card rounded-xl shadow-sm border border-border overflow-hidden">
                ...
                <div className="flex flex-col gap-3 p-4">
                  {mockRecentActivity.map((activity) => (
                    <div key={activity.id} className="flex items-center gap-4 p-3 rounded-lg border border-border/40 hover:bg-accent/40 bg-card/50">
